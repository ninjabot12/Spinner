...
var Program = class {
  constructor(gl2, {
    ...
    if (!gl2.canvas) console.error("gl not passed as fist argument to Program");
    this.gl = gl2;
    this.uniforms = uniforms;
    this.id = ID2++;
    if (!vertex3) console.warn("vertex shader not supplied");
    if (!fragment3) console.warn("fragment shader not supplied");
...
${addLineNumbers(fragment3)}`);
    ...
    if (!gl2.getProgramParameter(this.program, gl2.LINK_STATUS)) {
      return console.warn(gl2.getProgramInfoLog(this.program));
    }
    gl2.deleteShader(vertexShader);
    gl2.deleteShader(fragmentShader);
    this.uniformLocations = /* @__PURE__ */ new Map();
    let numUniforms = gl2.getProgramParameter(this.program, gl2.ACTIVE_UNIFORMS);
    for (let uIndex = 0; uIndex < numUniforms; uIndex++) {
      let uniform = gl2.getActiveUniform(this.program, uIndex);
      this.uniformLocations.set(uniform, gl2.getUniformLocation(this.program, uniform.name));
    ...
    const locations = [];
    const numAttribs = gl2.getProgramParameter(this.program, gl2.ACTIVE_ATTRIBUTES);
    for (let aIndex = 0; aIndex < numAttribs; aIndex++) {
      const attribute = gl2.getActiveAttrib(this.program, aIndex);
      const location = gl2.getAttribLocation(this.program, attribute.name);
...
function setUniform(gl2, type, location, value) {
  value = value.length ? flatten(value) : value;
  const setValue = gl2.renderer.state.uniformLocations.get(location);
  if (value.length) {
    if (setValue === void 0 || setValue.length !== value.length) {
      gl2.renderer.state.uniformLocations.set(location, value.slice(0));
    } else {
      if (arraysEqual(setValue, value)) return;
      setValue.set ? setValue.set(value) : setArray(setValue, value);
      gl2.renderer.state.uniformLocations.set(location, setValue);
    }
  } else {
    if (setValue === value) return;
    gl2.renderer.state.uniformLocations.set(location, value);
  }
  switch (type) {
    case 5126:
      return value.length ? gl2.uniform1fv(location, value) : gl2.uniform1f(location, value);
    case 35664:
      return gl2.uniform2fv(location, value);
    case 35665:
      return gl2.uniform3fv(location, value);
    case 35666:
      return gl2.uniform4fv(location, value);
    case 35670:
    case 5124:
    case 35678:
    case 35680:
      return value.length ? gl2.uniform1iv(location, value) : gl2.uniform1i(location, value);
    case 35671:
    case 35667:
      return gl2.uniform2iv(location, value);
    case 35672:
    case 35668:
      return gl2.uniform3iv(location, value);
    case 35673:
    case 35669:
      return gl2.uniform4iv(location, value);
    case 35674:
      return gl2.uniformMatrix2fv(location, false, value);
    case 35675:
      return gl2.uniformMatrix3fv(location, false, value);
    case 35676:
      return gl2.uniformMatrix4fv(location, false, value);
  }
}
...
var Camera = class extends Transform {
  constructor(gl2, { near = 0.1, far = 100, fov = 45, aspect = 1, left, right, bottom, top, zoom = 1 } = {}) {
    super();
...
var Texture = class {
  constructor(gl2, {
    ...
    unpackAlignment = 4,
    flipY = target == gl2.TEXTURE_2D ? true : false,
    anisotropy = 0,
    level = 0,
    width,
...
var RenderTarget = class {
  constructor(gl2, {
    ...
    if (depthTexture && (this.gl.renderer.isWebgl2 || this.gl.renderer.getExtension("WEBGL_depth_texture"))) {
      this.depthTexture = new Texture(gl2, {
        width,
        height,
        minFilter: this.gl.NEAREST,
        magFilter: this.gl.NEAREST,
        format: this.gl.DEPTH_COMPONENT,
        internalFormat: gl2.renderer.isWebgl2 ? this.gl.DEPTH_COMPONENT16 : this.gl.DEPTH_COMPONENT,
...
var Vec2 = class _Vec2 extends Array {
  ...
  add(va, vb) {
    if (vb) add3(this, va, vb);
    else add3(this, this, va);
    return this;
  }
  sub(va, vb) {
    if (vb) subtract2(this, va, vb);
    else subtract2(this, this, va);
    return this;
  }
  multiply(v) {
...
var Plane = class _Plane extends Geometry {
  constructor(gl2, { width = 1, height = 1, widthSegments = 1, heightSegments = 1, attributes = {} } = {}) {
...
var Box = class extends Geometry {
  constructor(gl2, { width = 1, height = 1, depth = 1, widthSegments = 1, heightSegments = 1, depthSegments = 1, attributes = {} } = {}) {
    const wSegs = widthSegments;
    const hSegs = heightSegments;
...
var Torus = class extends Geometry {
  constructor(gl2, { radius = 0.5, tube = 0.2, radialSegments = 8, tubularSegments = 6, arc = Math.PI * 2, attributes = {} } = {}) {
    ...
    for (let j = 0; j <= radialSegments; j++) {
      for (let i = 0; i <= tubularSegments; i++, idx++) {
        ...
        vertices.set([vertex3.x, vertex3.y, vertex3.z], idx * 3);
        center.x = radius * Math.cos(u);
        center.y = radius * Math.sin(u);
        normal.sub(vertex3, center).normalize();
...
var Post = class {
  ...
  addPass({ vertex: vertex3 = defaultVertex, fragment: fragment3 = defaultFragment, uniforms = {}, textureUniform = "tMap", enabled = true } = {}) {
    uniforms[textureUniform] = { value: this.fbo.read.texture };
    const program = new Program(this.gl, { vertex: vertex3, fragment: fragment3, uniforms });
...
var Skin = class extends Mesh {
  constructor(gl2, { rig, geometry, program, mode = gl2.TRIANGLES } = {}) {
    super(gl2, { geometry, program, mode });
...
function NormalProgram(gl2) {
  return new Program(gl2, {
    vertex,
    fragment,
    cullFace: null
  });
...
var Flowmap = class {
  constructor(gl2, {
    ...
    function createFBOs() {
      if (!type) type = gl2.HALF_FLOAT || gl2.renderer.extensions["OES_texture_half_float"].HALF_FLOAT_OES;
      let minFilter = (() => {
        if (gl2.renderer.isWebgl2) return gl2.LINEAR;
        if (gl2.renderer.extensions[`OES_texture_${type === gl2.FLOAT ? "" : "half_"}float_linear`]) return gl2.LINEAR;
        return gl2.NEAREST;
      })();
      const options = {
        width: size,
...
var GPGPU = class {
  ...
  addPass({ vertex: vertex3 = defaultVertex2, fragment: fragment3 = defaultFragment2, uniforms = {}, textureUniform = "tMap", enabled = true } = {}) {
...
var Polyline = class {
  constructor(gl2, {
    points,
    // Array of Vec3s
    vertex: vertex3 = defaultVertex3,
    fragment: fragment3 = defaultFragment3,
    uniforms = {},
    attributes = {}
    ...
    this.mesh = new Mesh(gl2, { geometry, program });
  }
  updateGeometry() {
    this.points.forEach((p, i) => {
...
var Shadow = class {
  constructor(gl2, { light = new Camera(gl2), width = 1024, height = width }) {
    this.gl = gl2;
    this.light = light;
  ...
  add({
    ...
    receive = true,
    cast = true,
    vertex: vertex3 = defaultVertex4,
    fragment: fragment3 = defaultFragment4,
    ...
    if (vertex3 === defaultVertex4 && fragment3 === defaultFragment4) {
...
var KTXTexture = class extends Texture {
  constructor(gl2, { buffer, wrapS = gl2.CLAMP_TO_EDGE, wrapT = gl2.CLAMP_TO_EDGE, anisotropy = 0 } = {}) {
    super(gl2, {
      generateMipmaps: false,
      wrapS,
      wrapT,
...
var TextureLoader = class {
  static load(gl2, {
    ...
    const support = this.getSupportedExtensions(gl2);
    let ext = "none";
    ...
    const cacheID = src + wrapS + wrapT + anisotropy + format + internalFormat + generateMipmaps + minFilter + magFilter + premultiplyAlpha + unpackAlignment + flipY + gl2.renderer.id;
    ...
    switch (ext) {
      ...
      case "png":
        ...
        texture.loaded = this.loadImage(gl2, src, texture);
        break;
      default:
  ...
  static getSupportedExtensions(gl2) {
    if (supportedExtensions.length) return supportedExtensions;
    const extensions = {
      pvrtc: gl2.renderer.getExtension("WEBGL_compressed_texture_pvrtc") || gl2.renderer.getExtension("WEBKIT_WEBGL_compressed_texture_pvrtc"),
      s3tc: gl2.renderer.getExtension("WEBGL_compressed_texture_s3tc") || gl2.renderer.getExtension("MOZ_WEBGL_compressed_texture_s3tc") || gl2.renderer.getExtension("WEBKIT_WEBGL_compressed_texture_s3tc"),
      etc: gl2.renderer.getExtension("WEBGL_compressed_texture_etc"),
      etc1: gl2.renderer.getExtension("WEBGL_compressed_texture_etc1"),
      astc: gl2.renderer.getExtension("WEBGL_compressed_texture_astc")
    };
  ...
  static loadImage(gl2, src, texture) {
    return decodeImage(src).then((imgBmp) => {
      if (!powerOfTwo(imgBmp.width) || !powerOfTwo(imgBmp.height)) {
        if (texture.generateMipmaps) texture.generateMipmaps = false;
        if (texture.minFilter === gl2.NEAREST_MIPMAP_LINEAR) texture.minFilter = gl2.LINEAR;
        if (texture.wrapS === gl2.REPEAT) texture.wrapS = texture.wrapT = gl2.CLAMP_TO_EDGE;
      }
...
var GLTFSkin = class extends Mesh {
  constructor(gl2, { skeleton, geometry, program, mode = gl2.TRIANGLES } = {}) {
    super(gl2, { geometry, program, mode });
...
var GLTFLoader = class {
  static async load(gl2, src) {
    ...
    gl2.renderer.bindVertexArray(null);
    const bufferViews = this.parseBufferViews(gl2, desc, buffers);
    const images = this.parseImages(gl2, desc, dir, bufferViews);
    const textures = this.parseTextures(gl2, desc, images);
    const materials = this.parseMaterials(gl2, desc, textures);
    const skins = this.parseSkins(gl2, desc, bufferViews);
    const meshes = this.parseMeshes(gl2, desc, bufferViews, materials, skins);
    const nodes = this.parseNodes(gl2, desc, meshes, skins);
    this.populateSkins(skins, nodes);
    const animations = this.parseAnimations(gl2, desc, nodes, bufferViews);
  ...
  static unpackGLB(glb) {
    ...
    if (jsonChunkHeader[1] !== 1313821514 || binaryChunkHeader[1] !== 5130562) {
  ...
  static parseBufferViews(gl2, desc, buffers) {
    ...
    desc.meshes && desc.meshes.forEach(({ primitives }) => {
      primitives.forEach(({ attributes, indices }) => {
        ...
        bufferViews[desc.accessors[indices].bufferView].target = gl2.ELEMENT_ARRAY_BUFFER;
      });
    });
    ...
    bufferViews.forEach(
      ({
        ...
        gl2.renderer.state.boundBuffer = buffer;
        gl2.bufferData(target, data, gl2.STATIC_DRAW);
        bufferViews[i].buffer = buffer;
      }
    );
    return bufferViews;
  }
  static parseImages(gl2, desc, dir, bufferViews) {
  ...
  static parseTextures(gl2, desc, images) {
    ...
    return desc.textures.map(({ sampler: samplerIndex, source: sourceIndex, name, extensions, extras }) => {
      ...
      const texture = new Texture(gl2, options);
      texture.name = name;
  ...
  static parseMaterials(gl2, desc, textures) {
  ...
  static parseSkins(gl2, desc, bufferViews) {
  ...
  static parseMeshes(gl2, desc, bufferViews, materials, skins) {
    ...
    return desc.meshes.map(
      ({
        ...
        desc.nodes && desc.nodes.forEach(({ mesh, skin }) => {
          if (mesh === meshIndex) {
            numInstances++;
            if (skin !== void 0) skinIndex = skin;
          }
        });
        primitives = this.parsePrimitives(gl2, primitives, desc, bufferViews, materials, numInstances).map(({ geometry, program, mode }) => {
          const mesh = typeof skinIndex === "number" ? new GLTFSkin(gl2, { skeleton: skins[skinIndex], geometry, program, mode }) : new Mesh(gl2, { geometry, program, mode });
          mesh.name = name;
  ...
  static parsePrimitives(gl2, primitives, desc, bufferViews, materials, numInstances) {
    return primitives.map(
      ({
        ...
        const geometry = new Geometry(gl2);
        ...
        const program = new NormalProgram(gl2);
  ...
  static parseNodes(gl2, desc, meshes, skins) {
  ...
  static parseAnimations(gl2, desc, nodes, bufferViews) {
